...
\begin{frame}{\insertsubsection}
	...
	\begin{itemize}
		...
		\item 訓練データ$\mathcal{D} = (X, \bm y) = \left\{ (\bm x_i, y_i) \right\}_{i=1}^t$に基づき予測モデルを更新する
		\item 予測モデルに基づき\alert{最適な点}を次に観測する
		\item 観測した$(x_{\rm next}, y_{\rm next})$を訓練データに追加し再び予測モデルを更新する
	\end{itemize}
...
\begin{frame}{\insertsubsection}
	\begin{itemize}
		...
		\begin{itemize}
			\item Regret = $\sum_{t=1}^T\{(\mbox{真の関数の最大値}) - (\mbox{観測済みデータの最大値})\}$
			\vspace{2pt}
			\item Average Regret : Regretをイテレーション数$T$で割ったもの
			\vspace{2pt}
